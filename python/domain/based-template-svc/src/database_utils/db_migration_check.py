#!/usr/bin/env python3
"""
Read-only migration health checker for based-template-svc (MySQL版).

- SQLAlchemy models(Base.metadata) vs MySQL上の実テーブルを比較
- 診断結果のみ表示（書き込みなし）
"""
...
def main() -> int:
    print("=" * 80)
    print("based-template-svc | Schema Health Check (READ ONLY / MySQL)")
    print("=" * 80)
    print("MySQL URL:", sync_engine.url)
    ...
    print()
    print("Schema Diff (Model vs MySQL)")
    print("-" * 40)
    ...
    if missing_in_db:
        print()
        print("[WARN] Tables defined in model but missing in MySQL:")
        for name in missing_in_db:
            print(" -", name)

    if extra_in_db:
        print()
        print("[INFO] Tables existing in MySQL but not in model:")
    ...
    for table_name in common_tables:
        ...
        db_cols = {c.name for c in db_metadata.tables[table_name].columns}
